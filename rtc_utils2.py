...
class WebRTCStreamer:
    ...
    async def initialize(self):
        ...
        self.media_tracks['audio'] = MediaStreamTrack(kind='audio')
        self.media_tracks['video'] = MediaStreamTrack(kind='video')
        
        for track in self.media_tracks.values():
    ...
    async def push_media(self, audio_data: bytes = None, video_data: bytes = None):
        """推送媒体数据"""
        if audio_data:
            await self.media_tracks['audio'].send(audio_data)
        if video_data:
            await self.media_tracks['video'].send(video_data)
