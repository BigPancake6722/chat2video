...
async def handle_offer(request: OfferRequest):
    print("offer请求", request)
    offer = RTCSessionDescription(sdp=request.sdp, type=request.type)
    session_id = str(randN(6))
    
    # 初始化 Player
    player = HumanPlayer(model_id=request.model_id, output_fps=VIDEO_FPS)
...
async def api_llm_tts_stream(request: TTSRequest):
    ...
    try:
        # 停止前一个推流任务
        if hasattr(player, '_stream_task'):
            player._stream_task.cancel()
        ...
        async def process_stream():
            player.stop_silent()
            async for text, sr, wav_data, opus_data in process_dual_tts_stream(tts_stream):
                # 异步生成视频帧
                ...
                player.feed_external_data(
                    ...
                    opus_audio=opus_data
                )
                asyncio.sleep(0.2)
            player.start_silent()

        # 保存任务引用以便取消
        player._stream_task = asyncio.create_task(process_stream())
        return {"text": "推流中"}
