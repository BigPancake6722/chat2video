...
def render_set(model_path, name, iteration, viewpoint_stack, gaussians, pipeline,audio_dir, batch_size, args):
    h264_stream = []
    ...
    if ksize > 0 and ksize % 2 == 1:
        ...
        erode = lambda x, y:x
    full_output_tensor = torch.empty((0, 3, 512, 512), device="cuda")
    with torch.no_grad():
        ...
        with ThreadPoolExecutor(max_workers=batch_size) as executor:
            for idx in tqdm(range(iterations), desc="Rendering image",total = iterations):
                ...
                full_output_tensor = torch.cat([full_output_tensor, output_tensor], dim=0)
        return tensor_to_image(full_output_tensor)
...
def render_sets(dataset : ModelParams, hyperparam, iteration : int, pipeline : PipelineParams, args, audio_arr=None):
    ...
    with torch.no_grad():
        ...
        if args.custom_aud != '':
            ...
            return render_set(dataset.model_path, "custom", scene.loaded_iter, scene.getCustomCameras(), gaussians, pipeline, audio_dir, batch_size, args)
        
        if not skip_train:
            audio_dir = os.path.join(data_dir, "aud.wav")
            return render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, audio_dir, batch_size, args)

        if not skip_test:
            audio_dir = os.path.join(data_dir, "aud.wav")
            return render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, audio_dir, batch_size, args)
...
def main(custom_cmd_list=None, audio_arr=None):
    """Original main function preserved as entry point"""
    ...
    # Parse arguments
    args = get_combined_args(parser, custom_cmd_list or [])
    ...
    print("总渲染帧数", video_images.shape)
    return video_images