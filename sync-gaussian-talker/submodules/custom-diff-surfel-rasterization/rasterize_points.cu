	...
	const int custom_channels)
{
  ...
  torch::Tensor out_color = torch::full({custom_channels, H, W}, 0.0, float_opts);
	...
	const int custom_channels) 
{
  ...
  torch::Tensor dL_dcolors = torch::zeros({P, custom_channels}, means3D.options());
