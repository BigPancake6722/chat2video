...
void FORWARD::render(
	...
	int custom_channels)
{
	auto launch_render = [&](auto Channels){
	renderCUDA<Channels> << <grid, block >> > (
		ranges,
		point_list,
		W, H,
		focal_x, focal_y,
	...
	switch (custom_channels) {
			case 1:
					launch_render(std::integral_constant<int, 1>{});
					break;
			case 3:
					launch_render(std::integral_constant<int, 3>{});
					break;
			case 4:
					launch_render(std::integral_constant<int, 4>{});
					break;
			case 5:
					launch_render(std::integral_constant<int, 5>{});
					break;
			default:
					break;
	};
...
void FORWARD::preprocess(int P, int D, int M,
	...
	int custom_channels)
{
	auto launch_preprocess = [&](auto Channels) {
			preprocessCUDA<Channels><<<(P + 255) / 256, 256>>>(
					P, D, M,
					means3D,
					scales,
					scale_modifier,
			...
			cudaDeviceSynchronize(); // 同步确保内核执行完成
	};
	switch (custom_channels) {
			case 1:
					launch_preprocess(std::integral_constant<int, 1>{});
					break;
			case 3:
					launch_preprocess(std::integral_constant<int, 3>{});
					break;
			case 4:
					launch_preprocess(std::integral_constant<int, 4>{});
					break;
			case 5:
					launch_preprocess(std::integral_constant<int, 5>{});
					break;
			default:
					break;
	};
