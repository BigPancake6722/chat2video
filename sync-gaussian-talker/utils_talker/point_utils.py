...
def downsample_point_cloud_cluster(points, voxel_size):
    ...
    # 获取下采样后的点云矩阵
    downsampled_pcb = voxel_down_sample_custom(points, voxel_size)
    downsampled_points = np.asarray(downsampled_pcb.points)
...
def depths_to_points(view, depthmap):
    c2w = (view.world_view_transform.T).inverse().to(depthmap.device)
    full_proj_transform = view.full_proj_transform.to(depthmap.device)
