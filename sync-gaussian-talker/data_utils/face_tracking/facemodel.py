...
class Face_3DMM(nn.Module):
    def __init__(self, modelpath, id_dim, exp_dim, tex_dim, point_num):
        ...
        for i in range(3):
            mu[:, i] -= np.mean(mu[:, i])
        mu = mu.reshape(-1)
        self.base_id = torch.as_tensor(base_id).cuda() /1e5
        self.base_exp = torch.as_tensor(base_exp).cuda() / 1e5
        self.mu = torch.as_tensor(mu).cuda() / 1e5
    ...
    def forward_geo_sub(self, id_para, exp_para, sub_index):
        ...
        sel_index = torch.cat((3*sub_index.unsqueeze(1), 
