...
def render_set(model_path, name, iteration, viewpoint_stack, gaussians, pipeline,audio_dir, batch_size, args, audio_arr, index=0):
    ...
    with torch.no_grad():
        ...
        with ThreadPoolExecutor(max_workers=batch_size) as executor:
            for idx in tqdm(range(iterations), desc="Rendering image",total = iterations):
                ...
                if videos is None:
                    videos = {}
                    videos["render"] = cv2.VideoWriter(f'{render_path}/renders{index}.mp4', fourcc, fps, (W, H))
...
def render_sets(dataset : ModelParams, hyperparam, iteration : int, pipeline : PipelineParams, args, audio_arr=None, index=0):
    ...
    with torch.no_grad():
        ...
        if args.custom_aud != '':
            ...
            render_set(dataset.model_path, "custom", scene.loaded_iter, scene.getCustomCameras(), gaussians, pipeline, audio_dir, batch_size, args, audio_arr=audio_arr, index=index)
        
        if not skip_train:
            audio_dir = os.path.join(data_dir, "aud.wav")
            render_set(dataset.model_path, "train", scene.loaded_iter, scene.getTrainCameras(), gaussians, pipeline, audio_dir, batch_size, args, audio_arr=None)

        if not skip_test:
            audio_dir = os.path.join(data_dir, "aud.wav")
            render_set(dataset.model_path, "test", scene.loaded_iter, scene.getTestCameras(), gaussians, pipeline, audio_dir, batch_size, args, audio_arr=None)
...
def main(custom_cmd_list, audio_arr, index):
    ...
    print(args)
    render_sets(model.extract(args), hyperparam.extract(args), args.iteration, pipeline.extract(args), args, audio_arr=audio_arr, index=index)